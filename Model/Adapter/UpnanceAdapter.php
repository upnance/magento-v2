<?php
namespace Upnance\Gateway\Model\Adapter;
...
/**
 * Class UpnanceAdapter
 */
class UpnanceAdapter
{
    const PUBLIC_KEY_XML_PATH      = 'payment/upnance_gateway/apikey';
    const TRANSACTION_FEE_XML_PATH = 'payment/upnance_gateway/transaction_fee';
    const AUTOCAPTURE_XML_PATH = 'payment/upnance_gateway/autocapture';
    const TEXT_ON_STATEMENT_XML_PATH = 'payment/upnance_gateway/text_on_statement';
    const PAYMENT_METHODS_XML_PATH = 'payment/upnance_gateway/payment_methods';
    const SPECIFIED_PAYMENT_METHOD_XML_PATH = 'payment/upnance_gateway/payment_method_specified';
    const BRANDING_ID_XML_PATH = 'payment/upnance_gateway/branding_id';
    const TEST_MODE_XML_PATH = 'payment/upnance_gateway/testmode';
    ...
    public function CreatePaymentLink($order, $area = \Magento\Framework\App\Area::AREA_FRONTEND)
    {
        try {
            ...
            if($order->getPayment()->getMethod() != \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_PAYPAL) {
                ...
                $mobileNumber = '';
                if($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_MOBILEPAY
                    || $order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE) {
            ...
            $form['shopsystem']['version'] = $this->moduleResource->getDbVersion('Upnance_Gateway');
            ...
            if($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_KLARNA) {
                $paymentMethods = 'klarna-payments';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_MOBILEPAY) {
                $paymentMethods = 'mobilepay';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_VIPPS){
                $paymentMethods = 'vipps,vippspsp';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_PAYPAL) {
                $paymentMethods = 'paypal';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_VIABILL) {
                $paymentMethods = 'viabill';
            }elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_SWISH) {
                $paymentMethods = 'swish';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_TRUSTLY) {
                $paymentMethods = 'trustly';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_ANYDAY) {
                $paymentMethods = 'anyday-split';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_APPLEPAY) {
                $paymentMethods = 'apple-pay';
            } elseif($order->getPayment()->getMethod() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_GOOGLEPAY) {
            ...
            $parameters = [
                ...
                "continueurl"        => $this->url->getUrl('upnancegateway/payment/returns', ['order' => $order->getIncrementId()]),
                "cancelurl"          => $this->url->getUrl('upnancegateway/payment/cancel', ['order' => $order->getIncrementId()]),
                "callbackurl"        => $this->url->getUrl('upnancegateway/payment/callback'),
            ...
            if($area == \Magento\Framework\App\Area::AREA_ADMINHTML){
                $parameters['continueurl'] = $this->storeManager->getStore($storeId)->getBaseUrl().'upnancegateway/payment/returns/order/'.$order->getIncrementId().'/area/'.$area;
                $parameters['cancelurl'] = $this->storeManager->getStore($storeId)->getBaseUrl().'upnancegateway/payment/cancel/order/'.$order->getIncrementId().'/area/'.$area;
                $parameters['callbackurl'] = $this->storeManager->getStore($storeId)->getBaseUrl().'upnancegateway/payment/callback';
            }
    ...
    public function CreateMobilePayPaymentLink($quote, $shippingMethod = false){
        try {
            ...
            $parameters = [
                ...
                "continueurl"        => $this->url->getUrl('upnancegateway/payment/returns'),
                "cancelurl"          => $this->url->getUrl('upnancegateway/payment/cancel'),
                "callbackurl"        => $this->url->getUrl('upnancegateway/payment/callback'),
    ...
    public function validateResponse($order, $payments, $type){
        ...
        if($paymentMethod == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_ANYDAY){
            if($status != self::STATUS_ACCEPTED_CODE){
                if($type == self::CAPTURE_CODE){
                    throw new \Magento\Framework\Exception\LocalizedException(__('Upnance: payment not captured'));
                } elseif($type == self::REFUND_CODE) {
                    throw new \Magento\Framework\Exception\LocalizedException(__('Upnance: payment not refunded'));
                }
            }
        } else {
                ...
                foreach ($paymentArray['operations'] as $operation) {
                    if (!empty($operation['qp_status_code']) && $operation['type'] == $type) {
                        if (in_array($operation['qp_status_code'], static::$errorCodes)) {
                            throw new \Magento\Framework\Exception\LocalizedException(__('Upnance: ' . $operation['qp_status_msg']));
                        }
                    }
                }
            } else {
                /** IK: we process validation errors from payment gateway */
                throw new \Magento\Framework\Exception\LocalizedException(new Phrase(__('Upnance') . ' ' . $this->_generateErrorMessageLine($paymentArray)));
            }
        }
    }
    ...
    public function allowedEmptyResponse() {
        return [
            \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_KLARNA
        ];
    }
}
