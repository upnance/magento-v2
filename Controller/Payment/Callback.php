...
{
    const PRIVATE_KEY_XML_PATH            = 'payment/upnance_gateway/private_key';
    const TESTMODE_XML_PATH               = 'payment/upnance_gateway/testmode';
    const TRANSACTION_FEE_LABEL_XML_PATH  = 'payment/upnance_gateway/transaction_fee_label';
    const AUTOCAPTURE_XML_PATH            = 'payment/upnance_gateway/autocapture';
    const TRANSACTION_FEE_SKU             = 'transaction_fee';
    const SEND_INVOICE_EMAIL_XML_PATH     = 'payment/upnance_gateway/send_invoice_email';
    const AUTOCAPTURE_TRUSTLY_XML_PATH    = 'payment/upnance_trustly/autocapture';
    ...
    /**
     * @var \Upnance\Gateway\Helper\Order
     */
     ...
     * @param \Upnance\Gateway\Helper\Order $orderHelper
     */
    public function __construct(
        ...
        \Upnance\Gateway\Helper\Order $orderHelper
    )
    {
        ...
        try {
            ...
            if ($checksum === $submittedChecksum) {
                ...
                if ($response->accepted === true) {
                    ...
                    if($order->getPayment()->getMethodInstance()->getCode() == \Upnance\Gateway\Model\Ui\ConfigProvider::CODE_TRUSTLY){
                    ...
                    if (isset($response->metadata->type) && $response->metadata->type === 'card') {
                        ...
                        } elseif(!empty($response->acquirer)) {
